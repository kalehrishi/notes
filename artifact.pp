...
class  demo::artifact (
  ...
  if($demo::username and $demo::password) {
    $args = "-u ${demo::username} -p '${demo::password}'"
  } elsif ($demo::netrc) {
    $args = '-m'
  }
  ...
  $cmd = "/opt/nexus-script/download-artifact-from-nexus.sh -a ${gav} -e ${packaging} ${$includeClass} -n ${demo::url} -r ${repository} -o ${output} ${args} -v"
